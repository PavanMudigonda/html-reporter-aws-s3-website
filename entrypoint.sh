...
# Delete history
COUNT=$( sh -c "aws s3 ls s3://${AWS_S3_BUCKET} --recursive --human-readable --summarize" | grep "PRE" | wc -l )
...
if (( COUNT > INPUT_KEEP_REPORTS )); then
  ...
  sh -c "aws s3 ls s3://${AWS_S3_BUCKET} --recursive --human-readable --summarize" | grep "PRE" | tail -n ${NUMBER_OF_FOLDERS_TO_DELETE} | `awk '{print $2}'` | while read -r line
    do
